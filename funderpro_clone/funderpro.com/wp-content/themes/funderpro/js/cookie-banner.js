...
class CookieBanner {
  ...
  init() {
    const accepted = this.getCookie(this.COOKIE_NAME)
    if (accepted === null) {
  ...
  onDecline() {
    this.setCookie(this.COOKIE_NAME, 'false', 365)
    this.hide()
  }

  accept() {
    const fn = window.on_cookie_notice_accept
    fn && fn()
  }
...
document.addEventListener('DOMContentLoaded', function() {
  const cookieBannerElement = document.querySelector('.site-cookie-notice')
